...
def main():
    ...
    if args.work_dir is not None and rank == 0:
        ...
        work_dir = os.path.join(('/').join(args.checkpoint.split('/')[:-1]),
                                'eval_results')
    ...
    if hasattr(model, 'presegmentor'):
        ...
        )  # to initial conv1/2 to get pseudo mask
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            metric = dataset.evaluate(
                results, seen_idx, unseen_idx,
                **eval_kwargs)  ##modify this part (Custom Evaluation)
