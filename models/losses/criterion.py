...
def cosine_margin_loss(q, e, labels, tau=1.0, m=0.5):
    ...
    pos = torch.exp(
        F.cosine_similarity(q, e[labels.long()].reshape(bs * n_cls, n_dim)) /
        tau)
    neg = torch.exp(
        F.cosine_similarity(q.unsqueeze(1), e.unsqueeze(0), dim=-1) / tau)
...
class SegPlusCriterion(nn.Module):
    ...
    def __init__(self, weight_dict, losses, eos_coef=0.1):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        target_masks = self._get_target_mask_binary_cross_entropy(
            src_masks, targets)
        ...
        target_masks_dice, valid = nested_tensor_from_tensor_list(
            masks_dice).decompose()
        ...
        src_masks_dice = F.interpolate(src_masks_dice[:, None],
                                       size=target_masks_dice.shape[-2:],
                                       mode="bilinear",
                                       align_corners=False)
        ...
        losses = {
            "loss_mask": sigmoid_focal_loss(src_masks, target_masks,
                                            num_masks),
            "loss_dice": dice_loss(src_masks_dice, target_masks_dice,
                                   num_masks),
        }
        return losses
